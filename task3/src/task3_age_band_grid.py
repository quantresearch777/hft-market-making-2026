...
def _scan_notional(path: Path, start_us: int, end_us: int) -> np.ndarray:
...
def compute_thresholds(paths: DataPaths, start: str, end: str, quantiles: list[float], sources: list[str]) -> pd.DataFrame:
    ...
    for source in sources:
        ...
        for symbol in SYMBOLS:
            if source in ("both", "binance"):
                arrays.append(_scan_notional(paths.liq_binance(symbol), start_us, end_us))
            if source in ("both", "bybit"):
                arrays.append(_scan_notional(paths.liq_bybit(symbol), start_us, end_us))
        values = np.concatenate(arrays).astype(float)
...
def parse_band(value: str) -> tuple[int, int]:
    ...
    left_s = int(left)
    right_s = int(right)
    if left_s < 0 or right_s <= left_s:
        raise argparse.ArgumentTypeError(f"Invalid band {value!r}; expected left:right seconds with 0 <= left < right")
...
def run(
    ...
    thresholds = compute_thresholds(paths, "2025-12-01", "2026-02-01", quantiles, sources)
    thresholds.to_csv(output_dir / "task3_age_band_thresholds.csv", index=False)

    max_horizon_us = max(HORIZONS) * 1_000_000
    max_band_right_us = max(right for _, right in bands) * 1_000_000
    rows = []
    ...
    for current in iter_dates(parse_date(start), parse_date(end)):
        ...
        for symbol in symbols:
            print(f"[age-band] {date_str} {symbol}", flush=True)
            ...
            liq_binance = load_liq(paths.liq_binance(symbol), start_us - max_band_right_us, end_us)
            liq_bybit = load_liq(paths.liq_bybit(symbol), start_us - max_band_right_us - BYBIT_DELAY_US, end_us)
            ...
            for threshold_row in thresholds.itertuples(index=False):
                source = str(threshold_row.source)
                ...
                for left_s, right_s in bands:
                    left_us = left_s * 1_000_000
                    right_us = right_s * 1_000_000
                    flags = ((age >= left_us) & (age < right_us)).astype(np.int8)
                    name = f"band_{source}_{q_label}_{left_s}s_{right_s}s"
    ...
    daily.to_csv(output_dir / "task3_age_band_daily_metrics.csv", index=False)
    summary.to_csv(output_dir / "task3_age_band_summary_by_split.csv", index=False)
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="reports/age_band_grid")
    parser.add_argument("--start", default="2025-12-01")
    parser.add_argument("--end", default="2026-03-01")
    parser.add_argument("--quantiles", nargs="+", type=float, default=[0.90, 0.95, 0.99])
    parser.add_argument(
        "--bands",
        nargs="+",
        type=parse_band,
        default=[parse_band(x) for x in ["0:10", "0:20", "0:30", "0:45", "0:60", "10:30", "20:60", "30:90", "60:120"]],
    )
