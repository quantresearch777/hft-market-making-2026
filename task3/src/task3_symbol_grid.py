...
def compute_symbol_thresholds(paths: DataPaths, start: str, end: str, quantiles: list[float], sources: list[str]) -> pd.DataFrame:
    ...
    for symbol in SYMBOLS:
        for source in sources:
            ...
            for q in quantiles:
                rows.append({"symbol": symbol, "source": source, "q": q, "threshold": float(np.quantile(values, q))})
...
def run(
    ...
    thresholds = compute_symbol_thresholds(paths, "2025-12-01", "2026-02-01", quantiles, sources)
    thresholds.to_csv(output_dir / "task3_symbol_thresholds.csv", index=False)

    threshold_lookup = {
        (row.symbol, row.source, row.q): float(row.threshold)
    ...
    for current in iter_dates(parse_date(start), parse_date(end)):
        ...
        for symbol in symbols:
            print(f"[symbol-grid] {date_str} {symbol}", flush=True)
            ...
            for source in sources:
                for q in quantiles:
                    ...
                    threshold = threshold_lookup[(symbol, source, q)]
                    ...
                    for window in windows:
                        ...
                        name = f"sym_{source}_{q_label}_win{window}"
    ...
    daily.to_csv(output_dir / "task3_symbol_daily_metrics.csv", index=False)
    summary.to_csv(output_dir / "task3_symbol_summary_by_split.csv", index=False)
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="reports/symbol_grid")
    parser.add_argument("--start", default="2025-12-01")
    parser.add_argument("--end", default="2026-03-01")
    parser.add_argument("--quantiles", nargs="+", type=float, default=[0.90, 0.95, 0.97, 0.99])
    parser.add_argument("--windows", nargs="+", type=int, default=[20, 30, 45, 60, 90])
    parser.add_argument("--sources", nargs="+", default=["both"], choices=["both", "binance", "bybit"])
