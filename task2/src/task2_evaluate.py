...
try:
    from task2_solution import BYBIT_DELAY_US, HORIZONS, LONG_WINDOW_US, WINDOW_US, predict
...
def add_markout_arrays(trades: pl.DataFrame, bbo: pl.DataFrame) -> tuple[np.ndarray, dict[int, np.ndarray], np.ndarray]:
    ...
    return weight, pnl_by_horizon, trade_ts
...
def score_one_day(
    ...
    flags_by_horizon = predict(trades, bbo, liq_binance, liq_bybit)
...
def summarize_daily(rows: list[dict], split_by_date: bool) -> pd.DataFrame:
    ...
    if split_by_date:
        ...
        group_cols = ["split", "horizon_s"]
    else:
        group_cols = ["horizon_s"]
    ...
    if split_by_date:
        ...
        result = result.assign(_split_order=result["split"].map(order)).sort_values(["_split_order", "horizon_s"]).drop(columns="_split_order")

    return result


def run(data_root: Path, output_dir: Path, start: str, end: str, symbols: tuple[str, ...]) -> None:
    ...
    for current in iter_dates(start_date, end_date):
        ...
        for symbol in symbols:
            print(f"[evaluate] {date_str} {symbol}", flush=True)
            ...
            liq_lookback_us = max(WINDOW_US, LONG_WINDOW_US)
            ...
            rows.extend(score_one_day(date_str, symbol, trades, bbo, liq_binance, liq_bybit))
    ...
    summary_by_split.to_csv(output_dir / "task2_summary_by_split.csv", index=False)
    summary_overall.to_csv(output_dir / "task2_summary_overall.csv", index=False)
    ...
    print("\nSummary by split", flush=True)
    print(summary_by_split.to_string(index=False), flush=True)
    print("\nOverall summary", flush=True)
    print(summary_overall.to_string(index=False), flush=True)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--data-root", required=True, help="Path to liquidation_task root or data directory.")
